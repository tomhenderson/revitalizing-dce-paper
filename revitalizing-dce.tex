...
DCE requires to map FILE operations to its internal custom handlers which work in synchronization with ns-3. DCE supports different operations on a FILE 
...
critical kernel 
components such as memory SLAB allocators through internal DCE callbacks mapped to the internal SLIB interface, 
task schedulers and timer functions, thereby providing the Linux network stack as library through an exported API.
