...
f3 = open("rate-vs-time.dce11.ns3.dat")
rate3 = []
clock3 = []
for line in f3:
    if not line.startswith('#'):
...
f5 = open("rate-vs-time.dce11.elf.user.dat")
rate5 = []
clock5 = []
for line in f5:
    if not line.startswith('#'):
...
plt.plot(rate4, clock4, marker='x', color='red', label='DCE-1.11')
plt.plot(rate, clock, marker='+', color='black', label='DCE-1.11 ELF')
plt.plot(rate5, clock5, marker='v', color='orange', label='DCE-1.11 ELF user')
